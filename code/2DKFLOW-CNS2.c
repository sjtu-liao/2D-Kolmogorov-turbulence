...
int main(int argc, char *argv[])
{
    ...
    char Char1[1000];
    char Char2[1000];
    mpfr_t PI, ZERO, Small, Ssmall, Sssmall, POINTONE, HALF, NEGONE, ONE, TWO, FOUR, NK, Re;
    mpfr_inits2(prec, tempreal, tempvir, tempreal2, tempvir2, temp, temp1, temp2, PI, ZERO, Small, Ssmall, Sssmall, POINTONE, HALF, NEGONE, ONE, TWO, FOUR, NK, Re, (mpfr_ptr) 0);
    ...
    /*initial*/
    mpfr_set_str(Ssmall, "1E-20", 10, GMP_RNDN);
    mpfr_set_str(Sssmall, "1E-40", 10, GMP_RNDN);
    for(n=0; n<evel; n++)
    {
        for(m=0; m<evel; m++)
        {
                        ...
                        mpfr_add(temp, temp1, temp2, GMP_RNDN);
                        mpfr_sin(temp, temp, GMP_RNDN);
                        mpfr_mul(temp, temp, Ssmall, GMP_RNDN);

                        mpfr_add(psi[m][n], psi[m][n], temp, GMP_RNDN);

                        mpfr_mul(temp, temp1, TWO, GMP_RNDN);
			            mpfr_add(temp, temp, temp2, GMP_RNDN);
                        mpfr_sin(temp, temp, GMP_RNDN);
                        mpfr_mul(temp, temp, Sssmall, GMP_RNDN);
            ...
            {
                {
                    ...
                    for(k=0; k<N; k++)
                    {
                        for(j=0; j<N; j++)
                        {
                            mpfr_sprintf(Char1, "%.262Re", CoeALL[NOWid[j][k]][NOWm[j][k]][NOWn[j][k]]);
                            mpfr_sprintf(Char2, "%.262Re", CoeALL[NOWid[j][k]][NOWm[j][k]][NOWn[j][k]+evel]);
                            fprintf(fp, "%d\t%d\t%.272s\t%.272s\n", k, j, Char1, Char2);
                        }
                    }
                    fclose(fp);
                }
